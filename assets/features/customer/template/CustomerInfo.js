...
export default function CustomerInfo({ customer }) {
  ...
  let customerActive = (
    <span className="app_badge app_badge--success">Membre actif</span>
  );
  if (!customer.membership.is_active) {
    customerActive = (
      <span className="app_badge app_badge--danger">Membre inactif</span>
    );
  }
      ...
      <div className="customer_row">
        <div className="customer_row__column--sm column_center__row">
          ...
          <img
            ...
            src={process.env.REACT_APP_SERVER_NAME + "/img/bggrey.jpg"}
            alt="logo"
          />
        </div>
