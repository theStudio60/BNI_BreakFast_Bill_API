...
class BillingPDFService
{
    ...
    public function getPDFBill(Bill $bill)
    {
        ...
        //file_put_contents($this->getBillFolder() . "/test.txt", "test");

        if (!is_dir($this->getBillFolder() . $customerFolder)) {
            mkdir($this->getBillFolder() . $customerFolder);
        }
        ...
        try {
            ...
            //on créer la facture
            // $pdf->Output("S", $this->getBillFolder() . $customerFolder . '/' . $nom_file);
            $res = $pdf->Output("S", $this->getBillFolder() . $customerFolder . '/' . $nom_file);
            file_put_contents($this->getBillFolder() . $customerFolder . '/' . $nom_file, $res);

            /* $pdf->Output() */
        } catch (\Throwable $th) {
            $message = $th->getMessage();
            file_put_contents($this->getBillFolder() . "/message.txt", $message);
            //throw $th;
        }
        return true;
    }
    ...
    public function getQrImage(Bill $bill)
    {
        ...
        $referenceNumber = QrPaymentReferenceGenerator::generate(
            ...
            "odjdojadadpaspdk" // A number to match the payment with your internal data, e.g. an invoice number
        );
